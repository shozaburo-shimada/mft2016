...
static void set_gpio(snd_seq_event_t*);
static void timer_handler();
static void led_timer_handler();
static void init_note_timer(void);
static void init_timer_signal(void);
static void init_led_timer(void);
static void init_led_timer_signal(void);
...
static unsigned char outputflag = 0;
static unsigned char speed_idx = 5;
static int note_cnt = 0;
static timer_t tid = 0;
static timer_t tid_led = 0;
struct sigaction act, oldact;

/*
 * RaspberryPi GPIO
 */
#define GPIO_BCM_LED 3
#define GPIO_BCM_21 21
#define BCM_SOFTPWM_PIN 16
...
static struct softPWMSET softpwm_settings [11]= {
  { 640, 320}, //x2^1 = x2倍速（最高速度）
  { 800, 480}, //x2^0.8
  { 960, 640}, //x2^0.6
  { 1120, 800}, //x2^0.4
  { 1280, 960}, //x2^0.2
  { 1440, 1120}, //x2^0 = x1倍速（標準速度）
  { 1600, 1280}, //x2^-0.2
  { 1760, 1440}, //x2^-0.4
  { 1920, 1600}, //x2^-0.6
  { 2080, 1760}, //x2^-0.8
  { 2240, 1920}, //2^-1 = 0.5倍速（最低速度)
};
...
static void init_gpio(void){
		if(wiringPiSetupGpio() < 0){
				fprintf(stderr, "Error: Cannot initialize gpio.\n");
				exit(1);
		}
		//GPIOのドライブ能力を最大に
		setPadDrive(0, 7);
		...
		//LED用
		pinMode(GPIO_BCM_LED, OUTPUT);
		digitalWrite(GPIO_BCM_LED, 0);

		//本番用
		pinMode(GPIO_BCM_21, OUTPUT);
		digitalWrite(GPIO_BCM_21, 0);
		outputflag = 0;
    softPwmCreate(BCM_SOFTPWM_PIN, 0, 100);
    softPwmWrite(BCM_SOFTPWM_PIN, 0);
...
static void set_gpio(snd_seq_event_t *ev){
	...
	switch (ev->type) {
		case SND_SEQ_EVENT_SYSEX:
				...
				if(outputflag == 1){
            softPwmStop(BCM_SOFTPWM_PIN);
            softPwmCreate(BCM_SOFTPWM_PIN, softpwm_settings[speed_idx].onvalue, softpwm_settings[speed_idx].pwmrange);
						softPwmWrite(BCM_SOFTPWM_PIN, softpwm_settings[speed_idx].onvalue);
				}

				digitalWrite(GPIO_BCM_LED, 1);
				if(tid_led != 0){
						timer_delete(tid_led);
				}
				init_led_timer();
...
static void init_led_timer_signal(){
		struct sigaction act_led, oldact_led;
		memset(&act_led, 0, sizeof(struct sigaction));
		memset(&oldact_led, 0, sizeof(struct sigaction));

		act_led.sa_handler = led_timer_handler;
		act_led.sa_flags = SA_RESTART;
		if(sigaction(SIGRTMIN, &act_led, &oldact_led) < 0){
				perror("sigaction()");
				exit(1);
		}
}
...
static void init_led_timer(void){
    ...
    itval.it_value.tv_sec = 0;
    itval.it_value.tv_nsec = 100000000; //100msec
    itval.it_interval.tv_sec = 0;
    itval.it_interval.tv_nsec = 0;

    //タイマの作成
    if(timer_create(CLOCK_REALTIME, &ev, &tid_led) < 0){
        perror("timer_create");
        exit(1);
    }
    //タイマーのセット
    if(timer_settime(tid_led, 0, &itval, NULL) < 0){
        perror("timer_settime");
        exit(1);
    }
    printf("Set timer, tid_led: %d\n", tid_led);
}
